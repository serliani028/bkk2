function Candidate() {
    ...
    this.initFilters = function () {
        ...
        $("#f_data_siswa").off();
        $("#f_data_siswa").change(function () {
            self.initCandidatesDatatable();
        });
        
        $("#status_siswa").off();
        $("#status_siswa").change(function () {
            self.initCandidatesDatatableSiswa();
        });
         $("#jurusan_siswa").off();
        $("#jurusan_siswa").change(function () {
            self.initCandidatesDatatableSiswa();
        });
        $("#tahun_angakatan").off();
        $("#tahun_angakatan").on('keyup', function() {
            self.initCandidatesDatatableSiswa();
            // alert('dsas');
        });
        ...
        ///////////////////////////////////////////////////////////////////////
        $("#id_sekolah_bkk").off();
        $("#id_sekolah_bkk").on('keyup', function() {
            self.initCandidatesDatatableSiswaBKK();
        });
         $("#jurusan_siswa_bkk").off();
        $("#jurusan_siswa_bkk").change(function () {
            self.initCandidatesDatatableSiswaBKK();
        });
        $("#tahun_siswa_bkk").off();
        $("#tahun_siswa_bkk").change(function() {
            self.initCandidatesDatatableSiswaBKK();
        });
    ...
    this.initCandidatesDatatable = function () {
        $('#candidates_datatable').DataTable({
            ...
            "ajax": {
                ...
                "data": function ( d ) {
                    d.status = $('#status').val();
                    d.jenis_mitra = $('.jenis_mitra').val();
                    d.f_siswa = $('#f_data_siswa').val();
                },
     ...
     this.initCandidatesDatatableSiswa = function () {
        $('#candidates_datatable_siswa').DataTable({
            "aaSorting": [[ 1, 'desc' ]],
    ...
    this.initCandidatesDatatableSiswaBKK = function () {
        $('#candidates_datatable_siswa_bkk').DataTable({
            "aaSorting": [[ 4, 'desc' ]],
            ...
            "ajax": {
                "type": "GET",
                "url": application.url+'/admin/sekolah/data_bkk',
                "data": function ( d ) {
                    d.tahun = $('#tahun_siswa_bkk').val();
                    d.id_sekolah = $('#id_sekolah_bkk').val();
                    d.jurusan = $('#jurusan_siswa_bkk').val();
                    // d.kelas = $('#kelas_siswa_bkk').val();
                },
                "complete": function (response) {
                    self.initiCheck(); 
                    self.initAllCheck();
    ...
    this.initCandidatesListBulkActionsSiswa = function () {
        $('.bulk-action-siswa').off();
        $('.bulk-action-siswa').on('click', function (e) {
            e.preventDefault();
            var ids = [];
            var action = $(this).data('action');
            $('.single-check').each(function (i, v) {
    ...
    this.initCandidatesListBulkActionsSiswaBKK = function () {
        $('.bulk-action-siswa-bkk').off();
        $('.bulk-action-siswa-bkk').on('click', function (e) {
            e.preventDefault();
            var ids = [];
            var action = $(this).data('action');
            $('.single-check').each(function (i, v) {
    ...
    this.initTugaskanCandidateForm = function(ids) {
         var modal = '#modal-default';
                ...
                application.load('admin/candidates/tugaskan-view', modal+' .modal-body-container', function (result) {
                    $("<input />").attr("type", "hidden").attr("name", "ids").attr("value", ids).appendTo('#admin_candidate_tugaskan_form');
                    self.initCKEditor();
...
$(document).ready(function() {
    ...
    candidate.initCandidatesDatatableSiswa();
    candidate.initCandidatesDatatableSiswaBKK();
    candidate.initCandidatesDatatable();
    candidate.initCandidatesListBulkActions();
    candidate.initCandidatesListBulkActionsSiswa();
    candidate.initCandidatesListBulkActionsSiswaBKK();
});
